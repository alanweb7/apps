...
import { IonicPage, NavController, Navbar ,NavParams,AlertController, Loading, LoadingController, ToastController,  ViewController } from 'ionic-angular';
...
export class MeusCodesPage {
  ...
  ionViewDidLoad() {
    ...
    this.meus_codes        = [];
    this.codes_serve       = [];
    console.log('ionViewDidLoad MeusCodesPage',this.token);
...
setFilteredItems() {
  ...
  ShowMenu(code){
    this.navCtrl.push('MenuCodePage', {token:this.token,code:code,qtd_img:this.package_imagens,qtd_videos:this.codes_videos,pacote:this.package_name});
  }
  getAllCode(token){
    if(this.net.ckeckNetwork()){
    ...
    .subscribe(
          (result: any) =>{
              ...
              console.log('RESULTADO',result.codes);
