...
import { IonicPage, NavController, NavParams , Slides, ModalController, Loading, LoadingController, ToastController, AlertController, ViewController} from 'ionic-angular';
...
export class MenuCodePage {
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad MenuCodePage');
    this.token      = String;
    this.code       = String;
    this.titulo     = String;
    ...
    this.qtd_img    = this.navParams.get('qtd_img');
    this.qtd_videos = this.navParams.get('qtd_videos');
    //this.getShowCode();
  }
              ...
              .subscribe(
                    (result: any) =>{
                      ...
                      if(result.status == 200){  
                            ...
                            console.log("tconteudo",this.meu_link);
                       ...
                       /*      for (let i = 0; i < this.vid_aux.length; i++) {
                              //descomentar
                             console.log("meus videos",this.vid_aux);
                              this.vid_aux[i]['video_link']= this.domSanitizer.bypassSecurityTrustResourceUrl(this.vid_aux[i]['video_link']);   
                              this.videos.push(this.vid_aux[i]);
                        } */
                            if(result.data[0]['tel_contato'] != ""){
                              console.log('entrei no phone',result.tel_contato);
                            ...
                            if(result.data[0]['tel_whatsapp']!= ""){
                                console.log('entrei no whatsapp');
  ...
  ShowContato(){
    this.navCtrl.push('ContatoCodePage', {contato:this.contato,token:this.token,code:this.id_code });
  }

  ShowCam(){
    this.navCtrl.push('ImageCodePage',{imagens:this.imagens,token:this.token,code:this.id_code,qtd:this.qtd_img,pacote:this.pacote});
  ...
  ShowVideo(){
    this.navCtrl.push('VideoCodePage',{videos:this.vid_aux,token:this.token,code:this.id_code,qtd:this.qtd_videos,pacote:this.pacote});
  ...
  editCode(){
    if(this.net.ckeckNetwork()){
      ...
      .subscribe(
            (result: any) =>{
              this.util.loading.dismiss(); 
              console.log("result code edit",result);
              if(result.status == 200){  
  ...
  change_segmento(item) {
    ...
    console.log("link",this.model.isLink);
  ...
  change_senha(item) {
    ...
    console.log("isprivate",this.modelG.isprivate);
    
  }
  // compartilhar social share
shareSheetShare(code) {
  this.socialSharing.share("Visite meu Code ->", "Share subject", "", "https://kscode.com.br/card?code="+code).then(() => {
